...
if(memberVision){memberVision.addEventListener('input',()=>visionCount.textContent=memberVision.value.length+' / 500');visionSubmit.addEventListener('click',async()=>{const idea=memberVision.value.trim();if(!idea){memberVision.focus();return}visionSubmit.disabled=true;visionSubmit.textContent='جارٍ الإرسال…';try{const r=await fetch('https://formsubmit.co/ajax/swiftwarmhealth@gmail.com',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({_subject:'رؤية عضو — نادي التوفيقية × SwiftWarm',message:idea,source:'SwiftWarm Tawfikeya app'})});if(!r.ok)throw new Error();visionThanks.classList.remove('hidden');memberVision.value='';visionCount.textContent='0 / 500';visionSubmit.textContent='تم استلام رؤيتك ✓'}catch(e){visionSubmit.disabled=false;visionSubmit.textContent='حاول مرة أخرى ←';visionThanks.textContent='تعذر الإرسال الآن. جرّب مرة أخرى.';visionThanks.classList.remove('hidden')}})}